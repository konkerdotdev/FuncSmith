  ...
  describe('lib', () => {
    describe('processFileSetItem', () => {
      it('should work as expected', async () => {
        ...
        const actual = await P.Effect.runPromise(unit.processFileSetItem(DEFAULT_MARKDOWN_OPTIONS)(item));
        expect(actual.contents).toEqual('<p>p1 content</p>\n');
      });

      it('should work as expected with non-matching globPattern', async () => {
        const item = fixturesFsFm.TEST_FILE_SET_FRONT_MATTER_1[1]!;
